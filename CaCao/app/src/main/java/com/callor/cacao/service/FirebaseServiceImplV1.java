 ...
 * 그 신호를 event 로 받아서 처리하는 클래스를 작성하기 위한 interface
 */
public class FirebaseServiceImplV1 implements ChildEventListener {

    /**
     * ChattAdapter 객체가 하는 일
     * RecyclerView 와 연동하여 데이터를 화면에 그리기 위한 중간 연결도구
     *
     * Adapter 를 MainActivity 로 부터 전달받아서
    ...
    /**
     * CUD 중에서 새로운 데이터가 추가되는(Insert, Create) event 가
     * 발생하면 실행되는 method
     *
     * 이 method 에서 데이터가 추가되었다는 신호를 받으면
     * 데이터를 가져와서 Adapter 의 chattList 에 데이터를 추가한다
